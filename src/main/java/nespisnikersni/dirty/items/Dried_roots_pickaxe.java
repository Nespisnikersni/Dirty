...
public class Dried_roots_pickaxe extends PickaxeItem {
    public Dried_roots_pickaxe(ToolMaterial material, int attackDamage, float attackSpeed, Settings settings) {
        super(material, attackDamage, attackSpeed, settings);
    }
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ItemStack itemStack = user.getStackInHand(hand);
        user.getItemCooldownManager().set(this,3600);
        StatusEffectInstance effect = new StatusEffectInstance(StatusEffects.HASTE, 1800, 0);
